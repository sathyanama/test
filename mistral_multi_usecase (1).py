...
def correct_text(input_text, task="rewrite", max_new_tokens=512):
    if task == "rewrite":
        prompt = f"Improve clarity, grammar, spelling, and tone. Make the writing suitable for goals or check-ins:\n\n{input_text}\n\nImproved version:"
    elif task == "summarize":
        prompt = f"Summarize the following into 4 to 5 clear and concise lines:\n\n{input_text}\n\nSummary:"
    elif task == "comms":
        prompt = f"Revise this technical message for grammar, clarity, and professionalism:\n\n{input_text}\n\nRevised communication:"
    else:
